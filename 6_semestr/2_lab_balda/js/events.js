var events = {
    ...
    answers: [], // полученные ответы
    bakArr: new Array(25), // ячейки букв
    track: [],
    numChar: null,
    scorePlayer: [], // подсчет действий игрока
    scoreBot: [], // подсчет действий бота
    result: '',
    error: '',
    start: function() {
        winners_list = JSON.parse(data) // Получаем базу выигрышей
        events.answers = ['балда']; // Начинаем со слова "балда"
        // Заполняем ячейки игового поля
    ...
    validate: function() {
        ...
        if (dic.findWord(result)) {
            ...
            if (events.numChar === null)
                ...
                document.getElementById("word_not_sup").className = ""
                document.getElementById("sup_yes").addEventListener('click',events.resumeGame,false)
                document.getElementById("sup_no").addEventListener('click',events.noresumeGame,false)
    ...
    resumeGame: function(){
        ...
        // Убираем предложение об изменении данных
        document.getElementById("word_not_sup").className = "hide"
        document.getElementById("sup_yes").removeEventListener('click',events.resumeGame,false)
        document.getElementById("sup_no").removeEventListener('click',events.noresumeGame,false)
        // вешаем обработчик выбора буквы
        document.getElementById('content').addEventListener('click', events.setChar, false);
        // удаляем задание пути
        document.getElementById('content').removeEventListener('click', events.setTrack, false);
        ...
        events.change();
        our3.dictonary.push(lastresult);
    },
    noresumeGame: function(){
        // Убираем предложение об изменении данных
        document.getElementById("word_not_sup").className = "hide"
        document.getElementById("sup_yes").removeEventListener()
        document.getElementById("sup_no").removeEventListener()
        events.change();
    },
    ...
    addWinner: function(scorePlayer,scoreBot){
        user_counts.push(scorePlayer)
        computer_counts.push(scoreBot)
        winners_list[0].User.push(scorePlayer)
        winners_list[0].Computer.push(scoreBot)
        var table = document.getElementById("table_of_counts");
    ...
    getWinners: function(){
        user_counts = winners_list[0].User
        computer_counts = winners_list[0].Computer
        document.getElementById('table_of_winners').className = ""
        var table = document.getElementById('table_of_counts')
        while(table.rows.length > 0) {
            table.deleteRow(0);
        }
        for(let i = 0; i < user_counts.length; i++){
            ...
            cell1.innerHTML = user_counts[i],
            cell2.innerHTML = computer_counts[i]
        }
        // JSON.push()
    }
};